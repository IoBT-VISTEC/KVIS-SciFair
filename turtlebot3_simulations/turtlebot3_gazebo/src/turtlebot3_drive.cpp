...
bool Turtlebot3Drive::init()
{
  ...
  model_state_sub_ = nh_.subscribe("gazebo/model_states", 1000, &Turtlebot3Drive::stateCallback, this);
...
void Turtlebot3Drive::odomMsgCallBack(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  // Which way that turtlebot3 is facing (angle)
...
void Turtlebot3Drive::stateCallback(const gazebo_msgs::ModelStates::ConstPtr &msg)
{
  int size = msg->pose.size();
  double xbot = msg->pose[size - 1].position.x;
  double ybot = msg->pose[size - 1].position.y;
  double zbot = msg->pose[size - 1].position.z;

  for (int i = 2; i < size - 1; i++)
  {
    double xtg = msg->pose[i].position.x;
    double ytg = msg->pose[i].position.y;
    double ztg = msg->pose[i].position.z;
    
    double distance = sqrt(pow((xtg - xbot), 2) + pow((ytg - ybot), 2) + pow((ztg - zbot), 2));
    ...
    if (distance < 0.5)
    {
      ...
      del.request.model_name = msg->name[i];
